...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd $PSScriptRoot; . .\.venv\Scripts\Activate.ps1; celery -A workers.celery_app worker --pool=solo --loglevel=info"

Write-Host "Starting Celery beat scheduler in a new window..."
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd $PSScriptRoot; . .\.venv\Scripts\Activate.ps1; celery -A workers.celery_app beat --loglevel=info"
